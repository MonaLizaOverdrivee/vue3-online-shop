  ...
  <div class="p-col-12 p-d-flex product-container  p-p-3">
    <div>
       <router-link :to="'/product/' + id" v-slot="{ navigate }"
          ><img :src="img" :alt="title" @click="navigate" class="img-product p-shadow-2"
        /></router-link>
    </div>
    <div class="content-container p-d-flex p-flex-column p-pl-3">
      <div class="p-d-flex p-jc-between p-pt-1">
        <div><h3 class="p-m-0">{{ title }}</h3></div>
        <div><span :class="quantityClass" v-text="quantityText"></span></div>
      </div>
      <div style="" class="discription-product">Lorem ipsum dolor sit amet consectetur, adipisicing elit. Error molestias accusantium mollitia beatae quas dolores quibusdam illo! Vel vitae veniam libero repellendus voluptas repellat vero sunt, debitis animi facilis ratione!</div>
      <div class="p-d-flex p-jc-between">
        <div><span class="product-price">{{ price }} РУБ</span></div>
